...
namespace Integrations.Pooling
{
    public class PoolingIntegration : ConnectorBase
    {
        ...
        public PoolingInfoDto GetInfoFor(Shipping shipping)
        {
            ...
            var slots = GetArr($"slots?dateFrom=2020-02-04&dateTo=2020-03-04&shippingRegionId={regionFromOrder.Id}&carType={carType.Id}&productType={filters.ProductTypes.First().Id}&clientId={client.Id}&carrierId={carrier.Id}");
            ...
            _poolingInfoDto = new PoolingInfoDto
            {
                ...
                MessageField = enumerable1.Any() ? "Эту перевозку можно отправить в Pooling" : "Нет доступного слота для этой перевозки",
        ...
        public CreateReservationResult CreateReservation(Shipping shipping)
        {
            ...
            try
            {
                ...
                createReservationResult.ReservationId = result.Get("$[0].slot.reservations[0].id").ElementAt(0).ToString();
                createReservationResult.ReservationNumber = result.Get("$[0].slot.reservations[0].number").ElementAt(0).ToString();
                return createReservationResult;
