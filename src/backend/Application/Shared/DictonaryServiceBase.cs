...
{
    public abstract class DictionaryServiceBase<TEntity, TListDto> where TEntity : class, IPersistable, new()
        where TListDto : IDto, new()
    {
        ...
        public ImportResultDto Import(IEnumerable<ValidatedRecord<TListDto>> entitiesFrom)
        {
            ...
            Log.Information("{entityName}.Import (Import): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
            sw.Restart();
            ...
            Log.Information("{entityName}.Import (Convert result to DTO): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
        ...
        private ImportResultDto MapFromImportResult(ImportResult importResult)
        {
            ...
            {
                sb.AppendLine("validation.duplicatedRecordErrorMessage".Translate(user.Language,
                    importResult.DuplicatedRecordErrorsCount));
            }

            if (importResult.InvalidDictionaryValueErrorsCount > 0)
            {
                sb.AppendLine("validation.invalidDictionaryValueErrorMessage".Translate(user.Language,
                    importResult.InvalidDictionaryValueErrorsCount));
            }

            if (importResult.InvalidValueFormatErrorsCount > 0)
            {
                sb.AppendLine("validation.invalidFormatErrorCountMessage".Translate(user.Language,
                    importResult.InvalidValueFormatErrorsCount));
            }

            if (importResult.RequiredErrorsCount > 0)
            {
                sb.AppendLine(
                    "validation.requiredErrorMessage".Translate(user.Language, importResult.RequiredErrorsCount));
            }
        ...
        protected DetailedValidationResult SaveOrCreateInner(TListDto dto, bool isImport)
        {
            ...
            Log.Information("{entityName}.SaveOrCreateInner (Find entity): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
            sw.Restart();
            ...
            Log.Information("{entityName}.SaveOrCreateInner (Apply updates): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
            sw.Restart();
            ...
            Log.Information("{entityName}.SaveOrCreateInner (Execure triggers): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
            sw.Restart();

            _dataService.SaveChanges();
            Log.Information("{entityName}.SaveOrCreateInner (Save changes): {ElapsedMilliseconds}ms", entityName,
                sw.ElapsedMilliseconds);
